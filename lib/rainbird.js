...
function RainbirdController(server, password, sensorDelay, retry, retryDelay) {
    if(!sensorDelay) {
        sensorDelay = 10;
    }
    if(!retry) {
        ...
        try {
            request(reqopts, function(error, response, body) {
                if(error || !response.statusCode || response.statusCode != 200) {
                    log('Request reply error: ' + JSON.stringify([error, response, body]));
                } else {
                    let decrypted = controller.decrypt(body, controller.password);
                    callback((JSON.parse(decrypted))['result']['data']);
                }
            });
            break;
        } catch(e) {
            log('Exception on request: ' + JSON.stringify([e,reqopts]));
        }
    }
};
...
RainbirdController.prototype.encode = function(cmd, args) {
    ...
    if(cmd_set) {
        ...
        if(args.length > cmd_set['length'] - 1) {
            log('Too much parameters for ' + request_cmd + ': ' + args.length + ', expected ' + cmd_set['length']);
            return '';
        }
        ...
        log('Command ' + request_cmd + ' is not available.');
        return '';
    }
};
...
RainbirdController.prototype.decode = function(data) {
    ...
    if(RAINBIRD_COMMANDS['ControllerResponses'][cmd_code]) {
        let cmd_tpl = RAINBIRD_COMMANDS['ControllerResponses'][cmd_code];
        let result = {
            type: cmd_tpl['type']
        };
        ...
        for(let key in cmd_tpl) {
            if(cmd_tpl.hasOwnProperty(key)) {
                if(undefined !== cmd_tpl[key]['position'] && cmd_tpl[key]['length']) {
                    ...
                    result[key] = parseInt(data.substr(_pos, _pos +_len), 16);
                }
            }
        }
...
RainbirdController.prototype.command = function(cmd, args, callback) {
    ...
    log('Requesting ' + cmd);
    this.request(data, RAINBIRD_COMMANDS['ControllerCommands'][cmd+'Request']['length'], function(response) {
        if(!response) {
            log('Empty response from client for ' + cmd);
            callback();
        }
        ...
        if(response.substr(0, 2) !== RAINBIRD_COMMANDS['ControllerCommands'][cmd+'Request']['response']) {
            log('Status request failed. Requested ' + RAINBIRD_COMMANDS['ControllerCommands'][cmd+'Request']['response'] + ' but got ' + response.substr(0, 2) + ': ' + decoded_response);
        }
...
RainbirdController.prototype.processCommand = function(cmd, args, callback) {
    this.command(cmd, args, function(response) {
        if(response && response['type'] == RAINBIRD_COMMANDS['ControllerCommands'][cmd+'Request']['response']['type']) {
            callback(response);
        } else {
            callback(response);
        }
...
RainbirdController.prototype.getSerialNumber = function(callback) {
    this.processCommand('SerialNumber', null, function(response) {
        callback(response['SerialNumber']);
    });
};
...
RainbirdController.prototype.cmdRunProgram = function(program, callback) {
    ...
    this.processCommand('ManuallyRunProgram', [program], function(response) {
        if(response['type'] == 'AcknowledgeResponse') {
...
RainbirdController.prototype.getRainDelay = function(callback)  {
    this.processCommand('RainDelayGet', null, function(response) {
        callback(response['delaySetting']);
    });
};
...
RainbirdController.prototype.cmdAdvanceZone = function(callback)  {
    this.processCommand('AdvanceStation', null, function(response) {
        if(response['type'] == 'AcknowledgeResponse') {
            callback(true);
        } else {
            callback(false);
...
RainbirdController.prototype.getCurrentIrrigation = function(callback)  {
    this.processCommand('CurrentIrrigationState', null, function(response) {
        callback(response['irrigationState'] ? true : false);
    });
};
