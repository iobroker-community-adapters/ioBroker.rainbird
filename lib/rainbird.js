	...
	try {
		request(reqopts, function(error, response, body) {
			if(error || !response.statusCode || response.statusCode != 200) {
				controller.context.log.info('Request reply error: ' + JSON.stringify([error, response, body]));
			} else {
				let decrypted = controller.decrypt(body, controller.password);
				let json = JSON.parse(decrypted);
				if(!json['result'] || !json['result']['data']) {
					controller.context.log.warn('Request returned unreadable data: ' + JSON.stringify([reqopts, json]));
				} else {
