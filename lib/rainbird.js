...
const RAINBIRD_COMMANDS = {
    ...
    ControllerResponses: {
        ...
        BB: {
            ...
            secondsRemaining: { position: 8, length: 4 },
            activeStation: { position: 16, length: 2 },
            running: { position: 22, length: 2 },
        },
    ...
    ControllerSpecialResponses: {
        3: {
            BB: {
                length: 10,
                type: 'CurrentRunTimeResponse',
                secondsRemaining: { position: 16, length: 4 },
                activeStation: { position: 12, length: 2 },
                running: { position: 6, length: 2 },
            },
        },
    },
    /*
...
/**
 * Create a keep-alive agent appropriate for the given protocol.
 * For HTTPS, certificate validation is disabled to support self-signed certs.
 */
...
function RainbirdController(server, password, protocol, context, sensorDelay, retry, retryDelay) {
    // default to HTTP; can be overridden via adapter config (controller.protocol)
    if (!protocol) {
        protocol = 'http';
    }
    this._agent = null; // cached keep-alive agent, created on first request
    if (!sensorDelay) {
        sensorDelay = 10;
    }
    if (!retry) {
...
RainbirdController.prototype.request = function (data, length, callback) {
    ...
    const request_id = Math.floor(new Date().getTime() / 1000);
    const send_data = {
        id: request_id,
        jsonrpc: '2.0',
        method: 'tunnelSip',
    ...
    // Reuse a single agent for connection pooling (created once, cached on instance)
    if (!this._agent) {
        this._agent = createAgent(this.protocol);
    }
    const keepAliveAgent = this._agent;

    const reqopts = {
        url: `${this.protocol}://${this.server}/stick`,
        // Disable certificate validation only for HTTPS (self-signed certs)
    ...
    try {
        request(reqopts, function (error, response, body) {
            if (error || !response.statusCode || response.statusCode != 200) {
                ...
                const decrypted = controller.decrypt(body, controller.password);
...
RainbirdController.prototype.addPadding = function (data) {
    ...
    const remain = BLOCK_SIZE - len;
    const padding = Math.abs(remain % BLOCK_SIZE);
    const str = PAD.repeat(padding);
    return result + str;
};
...
RainbirdController.prototype.encrypt = function (data, key) {
    ...
    const tocode_data = `${data}\x00\x10`;
    let m = crypto.createHash('sha256');
    m.update(key);
    const b = m.digest();

    const c = this.addPadding(tocode_data);

    m = crypto.createHash('sha256');
    m.update(data);
    const b2 = m.digest();

    const aes = crypto.createCipheriv(ALGORITHM, b, this.iv);
...
RainbirdController.prototype.decrypt = function (data, key) {
    ...
    const deciv = data.slice(32, 48);
    data = data.slice(48);

    const m = crypto.createHash('sha256');
    m.update(key);

    const sym_key = m.digest();

    const aes = crypto.createDecipheriv(ALGORITHM, sym_key, deciv);
...
RainbirdController.prototype.encode = function (cmd, args) {
    const request_cmd = `${cmd}Request`;
    const cmd_set = RAINBIRD_COMMANDS['ControllerCommands'][request_cmd];
    if (!args) {
        args = [];
    }

    if (cmd_set) {
        const cmd_code = cmd_set['command'];

        if (args.length > cmd_set['length'] - 1) {
            this.context.log.info(
                `Too many parameters for ${request_cmd}: ${args.length}, expected ${cmd_set['length']}`,
            );
            return '';
        }
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            const repeat = numlen - hexval.length;
            if (repeat > 0) {
...
RainbirdController.prototype.decode = function (data) {
    const controller = this;
    const devModelId = controller.deviceModelId;

    const cmd_code = data.substr(0, 2);
    if (RAINBIRD_COMMANDS['ControllerResponses'][cmd_code]) {
        let cmd_tpl;
        if (
            devModelId &&
            RAINBIRD_COMMANDS['ControllerSpecialResponses'][devModelId] &&
            RAINBIRD_COMMANDS['ControllerSpecialResponses'][devModelId][cmd_code]
        ) {
        ...
        for (const key in cmd_tpl) {
            // eslint-disable-next-line no-prototype-builtins
            if (cmd_tpl.hasOwnProperty(key)) {
                if (undefined !== cmd_tpl[key]['position'] && cmd_tpl[key]['length']) {
                    const _pos = cmd_tpl[key]['position'];
                    const _len = cmd_tpl[key]['length'];
                    result[key] = parseInt(data.substr(_pos, _len), 16);
                    controller.context.log.debug(
                        `Pos ${_pos} to ${_pos + _len - 1} from ${data} is ${result[key]} (${key})`,
                    );
                }
            }
        }
...
RainbirdController.prototype.command = function (cmd, args, callback) {
    const controller = this;
    const data = this.encode(cmd, args);
    ...
    this.request(data, RAINBIRD_COMMANDS['ControllerCommands'][`${cmd}Request`]['length'], function (error, response) {
        ...
        const decoded_response = controller.decode(response);
        if (response.substr(0, 2) !== RAINBIRD_COMMANDS['ControllerCommands'][`${cmd}Request`]['response']) {
            controller.context.log.warn(
                `Status request failed. Requested ${RAINBIRD_COMMANDS['ControllerCommands'][`${cmd}Request`]['response']} but got ${response.substr(0, 2)}: ${JSON.stringify(decoded_response)}`,
            );
            callback && callback();
        } else {
...
RainbirdController.prototype.processCommand = function (cmd, args, callback) {
    ...
    if (found !== false) {
        if (cmd.substr(0, 3) === 'cmd' || cmd.substr(0, 3) === 'set') {
            ...
            this.context.log.info(
                `Removing previous command from queue, it existed on earlier position: ${JSON.stringify([cmd, args])}`,
            );
        } else {
            this.context.log.info(
                `Not inserting command into queue, it already exists on earlier position: ${JSON.stringify([cmd, args])}`,
            );
            return;
        }
    }
...
RainbirdController.prototype.processQueue = function () {
    ...
    this.command(el.cmd, el.args, function (response) {
        ...
        if (
            response &&
            response['type'] === RAINBIRD_COMMANDS['ControllerCommands'][`${el.cmd}Request`]['response']['type']
        ) {
...
RainbirdController.prototype.getStatesArray = function (mask, len) {
    const repeat = len - mask.length;
    if (repeat > 0) {
        mask = '0'.repeat(repeat) + mask;
    }
    const cnt = mask.length * 4;
    const states = [];
    let rest = mask;
    while (rest) {
        const cur = parseInt(rest.substr(0, 2), 16);
        rest = rest.substr(2);
        for (let i = 0; i < 8; i++) {
...
RainbirdController.prototype.getAvailableStations = function (page, callback) {
    ...
    const numlen = RAINBIRD_COMMANDS['ControllerResponses']['83']['setStations']['length'];

    this.processCommand('AvailableStations', [page], function (response) {
        if (response) {
            const hexval = parseInt(response['setStations']).toString(16).toUpperCase();
            const states = controller.getStatesArray(hexval, numlen);
...
RainbirdController.prototype.getRainSensorState = function (callback, no_recheck) {
    ...
    if (
        !no_recheck &&
        (!this.sensorUpdateTime || this.sensorUpdateTime <= new Date().getTime() - this.sensorDelay * 1000)
    ) {
...
RainbirdController.prototype.getZoneState = function (zone, page, callback, no_recheck) {
    ...
    const numlen = RAINBIRD_COMMANDS['ControllerResponses']['BF']['activeStations']['length'];

    const controller = this;
    if (
        !no_recheck &&
        (!this.zonesUpdateTime || this.zonesUpdateTime <= new Date().getTime() - this.sensorDelay * 1000)
    ) {
        this.processCommand('CurrentStationsActive', [page], function (response) {
            if (response) {
                const hexval = parseInt(response['activeStations']).toString(16).toUpperCase();
                const states = controller.getStatesArray(hexval, numlen);
...
RainbirdController.prototype.getRunTime = function (callback) {
    ...
    this.processCommand('CurrentRunTime', [0], function (response) {
        if (response && response['type'] === 'CurrentRunTimeResponse') {
            const zone = parseInt(response['activeStation']);
            const runtime = response['secondsRemaining'];
