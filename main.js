...
const ioBLib = require('./lib/iob-lib').ioBLib;
...
function main() {
    ...
    ioBLib.setOrUpdateState(
        'device.commands.advanceZone',
        'Advance irrigation zone',
        false,
        '',
        'boolean',
        'button.next',
...
function pollStates() {
        ...
        controller.getAvailableStations(0, function (result) {
            if (result) {
                ...
                for (let i = 0; i < result.states.length; i++) {
                    s = i + 1;
                    const avail = result.states[i] ? true : false;
                    const idx = `device.stations.${s}.available`;
                    if (avail) {
                        ...
                        ioBLib.setOrUpdateState(
                            `device.stations.${s}.runZone`,
                            'Run zone for X minutes',
                            null,
                            '',
                            'number',
                            'level',
    ...
    controller.cmdWaterBudget(0, function (result) {
        if (result && 'adjust' in result) {
            ioBLib.setOrUpdateState(
                'device.settings.seasonalAdjust',
    ...
    controller.getZoneState(null, 0, function (result, runtime) {
        ...
        for (let i = 0; i < result.length; i++) {
            s = i + 1;
            const active = result[i] ? true : false;
            if (active) {
                irriStation = s;
            } else {
                if (availableStations.indexOf(s) > -1) {
                    ioBLib.setOrUpdateState(
                        `device.stations.${s}.remaining`,
                        `Remaining run time for station ${s}`,
                        0,
                        's',
                        'number',
                        'value',
            ...
            const idx = `device.stations.${s}.irrigation`;
            if (availableStations.indexOf(s) > -1) {
                ioBLib.setOrUpdateState(idx, `Station ${s} irrigation`, active, '', 'boolean', 'indicator.active');
            }
        }
        ioBLib.setOrUpdateState(
            'device.irrigation.station',
            'Irrigation on station',
            irriStation ? irriStation : 0,
            '',
            'number',
            'value.station',
        );
        if (runtime) {
            if (availableStations.indexOf(runtime['zone']) > -1) {
                ioBLib.setOrUpdateState(
                    `device.stations.${runtime['zone']}.remaining`,
                    `Remaining run time for station ${runtime['zone']}`,
                    runtime['seconds'],
                    's',
                    'number',
                    'value',
    ...
    if (id === 'device.commands.advanceZone') {
        ...
        if (found) {
            ...
            if (found && value > 0) {
                ...
                controller.cmdRunZone(found[1], value, function (_result) {
                    adapter.setState(id, null, true);
                    pollStates();
                });
            }
